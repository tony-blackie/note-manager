...
export default class EditTags extends React.Component<Props, State> {
    constructor(props) {
        super(props);
        this.state = {
            isOpen: null,
    ...
    handleInputChange(event) {
        ...
        let flterArr = arr.filter((item) => {
            return item.name.toLowerCase().substr(0, text.length) === text.toLowerCase();
        });
            ...
            <div>
                <div style={styles.wrapper}>
                   { this.state.mockData.map( (item, index) => 
                        <Chip key={item.id} style={styles.chip} onRequestDelete={()=>{this.handleRequestDelete(item.id)}}>{item.name}</Chip>
                    ) }
                </div>
                <IconMenu
                    ...
                    onRequestChange = { (e, r) => { (r === "clickAway") ? this.setState({isOpen:false}) : false } }
                    >
                    <div>
                        <input type="text" value={this.state.valueInput} onChange={(e)=>{this.handleInputChange(e)}}/>
                        { (this.state.searchArr.length === 0 && !this.state.trySearch) 
                            ? this.state.startData.map( (item, index) => 
                            <Checkbox key={index} label={item.name} onCheck={ ()=>{ this.updateCheck(item.id) } }/>) 
                            : this.state.searchArr.map( (item, index) => 
                            <Checkbox key={index} label={item.name} onCheck={ ()=>{ this.updateCheck(item.id) } }/>) 
                        }
                        {
                            (this.state.isVisibleBnt) ? <button onClick={()=>{this.saveData()}} >Save {this.state.valueInput}</button> : false
                        }
                    </div>
                </IconMenu>
            </div>
